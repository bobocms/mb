...
当前主域名：n0dlbnl5rz0xttao()
随机数字：fvpq24x31zpa5r5q(6)   6为长度，长度可自定义
随机字符：lcfhvepubhli6u6h(5)   5为长度，长度可自定义
...
/*以下是推送的URL地址，默认推送4条，可自定义增加或删减*/
    ...
    'http://'.$_SERVER['HTTP_HOST'].'/'.lcfhvepubhli6u6h(5),
    'http://'.$_SERVER['HTTP_HOST'].'/'.lcfhvepubhli6u6h(5).'/'.fvpq24x31zpa5r5q(6).'.html',
    'http://'.$_SERVER['HTTP_HOST'].'/'.lcfhvepubhli6u6h(5).'/'.lcfhvepubhli6u6h(5).'.html',
    'http://'.$_SERVER['HTTP_HOST'].'/'.lcfhvepubhli6u6h(5).'/'.lcfhvepubhli6u6h(5)
...
for($i=0;$i<2;$i++){
if(stripos($apiarr[$i],$_SERVER['HTTP_HOST'])!== false){
$api = $apiarr[$i];
$ch = curl_init();
$options =  array(
    CURLOPT_URL => $api,
